        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {agent.permissions.length === 0 ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Scope</th><th>Write</th><th>Audit Level</th></tr>
              </thead>
              <tbody>
      ...
      {tab === "logs" && (
        <div className="card" style={{ padding: 20 }}>
          ...
          {agent.logs.length === 0 ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Action</th><th>Model</th><th>Task</th><th>Time</th></tr>
              </thead>
              <tbody>
                {agent.logs.map((l) => (
                  <tr key={l.id}>
      ...
      {tab === "runs" && (
        <div className="card" style={{ padding: 20 }}>
          ...
          {agent.runs.length === 0 ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Status</th><th>Task</th><th>Cost</th><th>Started</th><th>Duration</th></tr>
              </thead>
              <tbody>
                {agent.runs.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      <td><span className={`badge ${r.status === "SUCCESS" ? "badge-green" : r.status === "FAILED" ? "badge-red" : ""}`} style={{ fontSize: 10 }}>{r.status}</span></td>
                      <td>{r.task ? <Link href={`/dashboard/tasks/${r.task.id}`} style={{ color: "var(--accent)" }}>{r.task.title}</Link> : "—"}</td>
                      <td>{r.cost != null ? `$${r.cost.toFixed(2)}` : "—"}</td>
                      <td className="muted" style={{ fontSize: 11 }}>{new Date(r.startedAt).toLocaleString()}</td>
