...
export function DisputeDetail({
  ...
  return (
    <DetailLayout
      ...
      meta={
        <>
          <span>{t("Filed:")} {new Date(dispute.createdAt).toLocaleString()}</span>
          {dispute.resolvedAt && <span>{t("Resolved:")} {new Date(dispute.resolvedAt).toLocaleString()}</span>}
          {dispute.windowEndsAt && <span>{t("Window ends:")} {new Date(dispute.windowEndsAt).toLocaleDateString()}</span>}
        </>
      }
    >
      {dispute.resolution && (
        <div className="card p-18" style={{ borderLeft: "3px solid var(--green)" }}>
          <h3 className="mt-0 mb-4">{t("Resolution")}</h3>
      ...
      {dispute.pob && (
        <div className="card p-18">
          <h3 className="mt-0 mb-12">{t("Related PoB Record")}</h3>
          ...
          {dispute.pob.attributions.length > 0 && (
            <>
              <h4 className="mt-16 mb-8">{t("Attribution")}</h4>
      ...
      {isAdmin && status === "OPEN" && (
        <div className="card p-18">
          ...
          <label className="field">
            <span className="label">{t("Resolution note")}</span>
            <textarea
              value={resolution}
      ...
      {auditLogs.length > 0 && (
        <div className="card p-18">
          <h3 className="mt-0 mb-12">{t("Audit Trail")}</h3>
