...
export const metadata = dashboardMeta("Pipeline Quality", "Node scorecard dimension");
...
export default async function PipelineQualityPage() {
  ...
  const avgScore =
    scorecards.length > 0
      ? scorecards.reduce((sum, sc) => sum + sc.pipelineScore, 0) / scorecards.length
      : 0;
  ...
  return (
    <div className="dashboard-page section">
      <div className="container-wide">
        ...
        <h1><T>Pipeline Quality</T></h1>
        <p className="muted"><T>Ranked by pipeline quality scores.</T></p>
                ...
                <tbody>
                  {scorecards.map((sc, idx) => (
                    <tr key={sc.id}>
                      ...
                      <td className="font-bold">{sc.pipelineScore.toFixed(1)}</td>
