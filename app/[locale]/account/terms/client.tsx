...
export default function TermsAcceptanceClient() {
  ...
  const allAccepted = REQUIRED_TERMS.every((doc) =>
    accepted.has(`${doc.type}:${doc.version}`)
  );
  ...
  return (
    <main className="section">
      <div className="container" style={{ maxWidth: 560 }}>
        <span className="eyebrow">{t("Onboarding")}</span>
        <h1>{t("Terms & Agreements")}</h1>
        <div className="card" style={{ marginTop: 18 }}>
          <p className="muted" style={{ marginBottom: 20 }}>
            {t("Please review and accept the following agreements before continuing.")}
          </p>
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <strong style={{ fontSize: 14 }}>{t(doc.label)}</strong>
                    <span className={`badge ${isAccepted ? "badge-green" : ""}`} style={{ fontSize: 10 }}>
                      {isAccepted ? t("Accepted") : `v${doc.version}`}
                    </span>
                  </div>
          ...
          <div style={{ marginTop: 20, display: "flex", gap: 10, justifyContent: "flex-end" }}>
            {!allAccepted && (
              <button className="button" onClick={handleAcceptAll} disabled={busy}>
                {busy ? t("Processing...") : t("Accept All")}
              </button>
            )}
            {allAccepted && (
